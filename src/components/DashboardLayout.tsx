...
export function DashboardLayout({ role, title, children }: { role: "student" | "instructor" | "admin"; title: string; children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      {/* Sidebar */}
      <aside className={`fixed md:static inset-y-0 left-0 z-30 w-64 bg-sidebar border-r border-sidebar-border flex flex-col transition-transform ${open ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        ...
        <div className="px-3 py-4 flex-1 overflow-y-auto">
          <div className="px-2 mb-3 text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground">{role} workspace</div>
          ...
          <div className="mt-8 px-2 text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground">Switch role</div>
          <div className="mt-2 space-y-1">
            {(["student", "instructor", "admin"] as const).filter((r) => r !== role).map((r) => (
              <Link key={r} to={`/${r}`} className="block rounded-md px-3 py-2 text-sm capitalize text-muted-foreground hover:bg-accent hover:text-foreground transition">{r} →</Link>
            ))}
          </div>
        </div>
        <div className="p-3 border-t border-sidebar-border">
          <div className="rounded-md bg-accent/50 p-3 text-xs">
            <div className="font-mono text-[10px] uppercase tracking-wider text-acid">Pro tip</div>
            <div className="mt-1 text-muted-foreground leading-relaxed">All metrics update live every few seconds.</div>
          </div>
        </div>
      </aside>
